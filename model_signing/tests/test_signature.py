...
def __get_stmnt() -> statement.Statement:
    return statement.Statement(
        subjects=[
            res_desc.ResourceDescriptor(name='abc', digest={'myhash': b'12345'}).pb,
            res_desc.ResourceDescriptor(name='def', digest={'myhash': b'67890'}).pb,
            res_desc.ResourceDescriptor(name='ghi', digest={'myhash': b'11111'}).pb,
        ],
...
def test_key_signature_success(tmp_path: pathlib.Path):
    ...
    signer = signing.KeySigner(priv_key_path)
    verifier = verifying.KeyVerifier.from_path(pub_key_path)
...
def test_key_signature_wrong_key(tmp_path: pathlib.Path):
    ...
    signer = signing.KeySigner(priv_key_path)
    verifier = verifying.KeyVerifier(_PUB_KEY_2)
