...
def pae(statement: statement_pb.Statement) -> bytes:
        ...
        pae = f'DSSEV1 {len(PAYLOAD_TYPE)} {PAYLOAD_TYPE} {payload_len} {enc_payload}'.encode()
        return pae


def load_single_cert(path: str) -> x509.Certificate:
        with open(path, 'rb') as fd:
                cert = x509.load_pem_x509_certificate(fd.read())
        return cert


def load_multiple_certs(paths: list[str]) -> list[x509.Certificate]:
        certs = b''
        for p in paths:
